...
<div class="position-relative overflow-hidden radial-gradient min-vh-100 d-flex flex-column">
  <div class="d-flex align-items-center justify-content-center flex-grow-1">
    ...
    <form action="{{route('updateCommissiom', $commission->id)}}" method="POST">
        @csrf
        @method('PUT')
        <div class="form-group">
            <label for="nom">Nom de la Commission</label>
            <input type="text" name="nom" class="form-control" id="nom" value="{{ $commission->nom }}" required>
        </div>
        <div class="form-group">
            <label for="mandat">Mandat</label>
            <textarea name="mandat" class="form-control" id="mandat" rows="2" required>{{ $commission->mandat }}</textarea>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <input type="date" name="date_expiration" class="form-control" id="date_expiration" value="{{ $commission->date_expiration }}" required>
        </div>
