      ...
      <div class="col-md-4 col-lg-4 col-xxl-4 mt-6 " style="margin-top: 150px;">
        <div class="card mb-2">
          <div class="card-body">
            ...
            @endif
            <form  method="post"  action="{{route('updateAvis',$editer->id)}}" enctype="multipart/form-data">
                @csrf
                @method('put')
                <div class="form-group">
                    <label for="composition_dossier">Composition du Dossier de Candidature :</label>
                    <textarea class="form-control" id="composition_dossier" name="composition_dossier" rows="2" required>{{$editer->composition_dossier}}</textarea>
                </div>

                <div class="form-group">
                    <label for="depot_candidature">Dépôt de Candidature :</label>
                    <textarea class="form-control" id="depot_candidature" name="depot_candidature" rows="2" required>{{$editer->depot_candidature}}</textarea>
                </div>
              <div class="mb-2">
                <label for="date_open" class="form-label">Date du Debut</label>
                <input type="date" class="form-control" id="datedebut" name="datedebut" name="{{$editer->datedebut}}">
              </div>
              <div class="mb-2">
                <label for="date_fin" class="form-label">Date de fin</label>
                <input type="date" class="form-control" id="datefin" name="datefin" name="{{$editer->datefin}}">
              </div>
